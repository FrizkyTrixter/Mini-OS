...
/* Real stack-switch scheduler test tasks */
...
static void shell_execute(char *cmd) {
    if (strcmp(cmd, "help") == 0) {
        ...
        print("  ptest       - run allocate/write/free test\n");
        print("  ticks       - show timer tick count\n");
        print("  sleep N     - sleep N ms\n");
        print("  hz          - check PIT frequency\n");
        print("  schedtest   - start A/B scheduler test\n");
        print("  test_div0   - trigger divide by zero\n");
        print("  test_pf     - trigger page fault\n");
        print("  test_ud     - trigger invalid opcode\n");
        print("  test_bp     - trigger breakpoint\n");
    }
        ...
        if (!page) {
            ...
            *p = 0xCAFEBABE;
            ...
            print("ptest: freed 0x");
            print_hex(page);
            print("\n");
        }
    }
    else if (strcmp(cmd, "ticks") == 0) {
        print_dec((uint32_t)timer_jiffies());
        print("\n");
    }
    else if (
        ...
        print("Waited ");
        print_dec((uint32_t)diff);
        print(" ticks (~");
        print_dec((uint32_t)(diff * 10));
        print(" ms)\n");
    }
    else if (strcmp(cmd, "schedtest") == 0) {
        print("Adding scheduler test tasks A and B...\n");
        scheduler_add_task(task_a);
        scheduler_add_task(task_b);
        print("Starting scheduler...\n");
        scheduler_start();

        /* scheduler_start should not return during this test */
        print("scheduler_start returned unexpectedly.\n");
    }
